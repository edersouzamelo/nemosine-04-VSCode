...
export function emptyStateCopy(kind: "no-audits" | "no-results" | "diagnostic") {
  ...
  return {
    title: "Nenhuma execução cognitiva auditada foi registrada neste ambiente.",
    body: "O runtime pode estar em modo off, a migração pode não ter sido aplicada ou ainda não houve uma execução em shadow/enforce.",
  };
}
